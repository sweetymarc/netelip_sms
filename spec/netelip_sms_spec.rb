...
describe NetelipSms do
  it "should return true when sending correctly a message" do
    ...
    NetelipSms.send_sms(:login => "foo", :password => "bar", :destination => "+34666666666", :message => "Lore ipsum", :from => "12345").should == true
  end
  ...
  it "should return response code when message was not sent (code != 200)" do
    ...
    Net::HTTP.should_receive(:post_form).with(NetelipSms::APIUri, anything).and_return(response)
    NetelipSms.send_sms(:login => "foo", :password => "bar", :destination => "+34666666666", :message => "Lore ipsum", :from => "12345").should == "401"
  end

  it "should raise an ArgumentError if no login present" do
    proc { NetelipSms.send_sms(:password => "bar", :destination => "+34666666666", :message => "Lore ipsum", :from => "12345") }.should raise_exception(ArgumentError, "Login must be present")
  end

  it "should raise an ArgumentError if no password present" do
    proc { NetelipSms.send_sms(:login => "foo", :destination => "+34666666666", :message => "Lore ipsum", :from => "12345") }.should raise_exception(ArgumentError, "Password must be present")
  end

  it "should raise an ArgumentError if no sender present" do
    proc { NetelipSms.send_sms(:login => "foo", :password => "bar", :destination => "+34666666666", :message => "Lore ipsum") }.should raise_exception(ArgumentError, "Sender must be present (from)")
  end

  it "should raise an ArgumentError if sender is more than 11 characters" do
    proc { NetelipSms.send_sms(:login => "foo", :password => "bar", :destination => "+34666666666", :message => "Lore ipsum", :from => "012345678901") }.should raise_exception(ArgumentError, "Sender length 11 characters maximum")
  end
  ...
  it "should accept login, password, sender for module configuration" do
    ...
    proc { NetelipSms.send_sms(:message => "Lorem Ipsum", :destination => "+34666666666").should == true }.should_not raise_exception(ArgumentError)
  end
end
